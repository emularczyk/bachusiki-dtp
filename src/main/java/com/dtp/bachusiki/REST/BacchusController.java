...
public class BacchusController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Bacchus> getBacchusById(@PathVariable Long id) {
    ...
    @PostMapping("/create")
    public ResponseEntity<List<Bacchus>> createBacchus(@RequestBody Bacchus createdBacchus) {
        try {
            ...
            List<Bacchus> bacchusList = bacchusService.createBacchus(createdBacchus.getName(), createdBacchus.getLocation(), createdBacchus.getAuthor(), createdBacchus.getSponsor(), createdBacchus.getDate());
            if (!bacchusList.isEmpty()) {
                return ResponseEntity.ok(bacchusList);
            } else {
                return ResponseEntity.noContent().build();
    ...
    public ResponseEntity<List<Bacchus>> updateBacchus(@PathVariable Long id, @RequestBody Bacchus updatedBacchus) {
        try {
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            LocalDate parsedDate = LocalDate.parse(updatedBacchus.getDate(), formatter);
            System.out.println("Valid date: " + parsedDate);

            List<Bacchus> bacchusList = bacchusService.updateBacchus(id, updatedBacchus.getName(), updatedBacchus.getLocation(), updatedBacchus.getAuthor(), updatedBacchus.getSponsor(), updatedBacchus.getDate());
            if (!bacchusList.isEmpty()) {
                return ResponseEntity.ok(bacchusList);
            } else {
                return ResponseEntity.noContent().build();
    ...
    public ResponseEntity<List<Bacchus>> deleteBacchus(@PathVariable Long id) {
